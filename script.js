...
/*****************************************************************/
	...
	function Player() {
		...
		this.getBank = function() {
			$('#bank').html('Voitot: $' + bank.formatMoney(2, '.', ','));

			if(bank < 0) {
				$('#bank').html('Voitot: <span style="color: #D90000">-$' + 
	...
	function Game() {
		this.newGame = function() {
			...
			if(player.checkWager()) {
				...
				showAlert('Ei riitä rahat moiseen!');
			}
		};
	}
...
/*****************************************************************/
	...
	Player.prototype.dbl = function() {
		...
		if(this.checkWager(wager * 2)) {
			...
			$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
			showAlert('Sinulla ei ole tarpeeksi rahaa alas tuplaamiseen!');
		}
	};

	Player.prototype.split = function() {
		$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
		showAlert('Lohkaisu-funktio ei vielä toimi.');
	};
	...
	Player.prototype.insure = function() {
		...
		if(this.checkWager()) {
			...
			this.setWager(-wager);
			$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
			showAlert('Sinulla ei ole tarpeeksi rahaa vakuutukseen!');
		}
	};
	...
	function getWinner() {
		...
		if(pscore > dscore) {
			...
			if(pscore <= 21 && dscore > 21) {
				...
				player.setBank(winnings - wager);
				$('#alert').removeClass('alert-info alert-error').addClass('alert-success');
				result = 'Sinä voitit - jakajan tuho!';
			} else if(dscore <= 21) {
				winnings -= wager;
				player.setBank(winnings);
				$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
			...
			if(pscore <= 21) {
				if(dscore === 21 && dhand.length < 3 && phand.length > 2) {
					...
					player.setBank(winnings);
					$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
					result = 'Sinä hävisit - jakajan Mustajaakko!';
				} else {
					winnings = wager;
	...
	$('#deal').on('click', function() {
		...
		if(cash > 0 && !running) {
			if($.trim($('#wager').val()) > 0) {
				...
				$('#alert').removeClass('alert-info alert-success').addClass('alert-error');
				showAlert('Rahaa pöytään! Ymmärrätkö?');
			}
		} else {
			$('#myModal').modal();
		}
